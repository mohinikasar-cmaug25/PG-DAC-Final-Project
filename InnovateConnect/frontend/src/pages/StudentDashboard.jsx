...
import { FiGithub, FiBriefcase, FiPlusCircle, FiList, FiCheckCircle, FiUpload, FiX, FiEdit, FiTrash2 } from 'react-icons/fi';
...
const StudentDashboard = ({ initialTab = 'internships' }) => {
    ...
    const getApplicationStatus = (internshipId) => {
        const app = myApplications.find(a => a.internshipId === internshipId);
    ...
    const handlePostIdea = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            if (isEditingIdea) {
                await axios.put(`http://localhost:5098/api/ideas/${editIdeaId}`, { ...ideaFormData, id: editIdeaId }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleEditIdeaClick = (idea) => {
        setIdeaFormData({
            title: idea.title || idea.Title,
            technologyUsed: idea.technologyUsed || idea.TechnologyUsed,
            description: idea.description || idea.Description
        });
        setIsEditingIdea(true);
        setEditIdeaId(idea.id || idea.Id);
        setActiveTab('post-idea');
    };
    ...
    const handleDeleteIdea = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5098/api/ideas/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md mb-8 flex flex-col md:flex-row justify-between items-center border border-gray-100">
                ...
                <div className="flex gap-6 mt-4 md:mt-0">
                    <div className="flex flex-col items-center">
                        ...
                        {(studentProfile?.resumeFileName || studentProfile?.ResumeFileName) && (
            ...
            <div className="flex gap-4 mb-6 border-b border-gray-200">
                ...
                >
                    {isEditingIdea ? 'Edit Idea' : 'Post New Idea'}
                </button>
            </div>
                    ...
                    {internships.map(item => {
                        ...
                        return (
                            <div
                                ...
                                className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                            >
                                ...
                                {status ? (
                                    <button
                                        disabled
                                        className={`w-full py-2.5 rounded-lg font-bold transition-colors uppercase text-sm tracking-wide ${status === 'Accepted' ? 'bg-green-600 text-white' :
            ...
            {selectedInternship && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/50 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transform transition-all p-8 border border-gray-100 relative">
                        <button
                        ...
                        <div className="flex items-start gap-5 mb-8">
                            ...
                            <div>
                                <h2 className="text-3xl font-bold text-dark mb-1">{selectedInternship.title}</h2>
                                <p className="text-lg text-primary font-semibold">{selectedInternship.company?.companyName || 'Unknown Company'}</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4 mb-8">
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                <p className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">Stipend</p>
                                <p className="text-xl font-bold text-dark">${selectedInternship.stipend}/mo</p>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                <p className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">Posted On</p>
                                <p className="text-lg font-semibold text-dark">{new Date(selectedInternship.postedDate).toLocaleDateString()}</p>
                            </div>
                        </div>

                        <div className="mb-8">
                            <h4 className="text-lg font-bold text-dark mb-3">About the Internship</h4>
                        ...
                        <div className="mb-8">
                            <h4 className="text-lg font-bold text-dark mb-3">Required Technologies</h4>
                            <div className="flex flex-wrap gap-2">
                                {selectedInternship.technologyUsed.split(',').map((tech, i) => (
                                    <span key={tech + i} className="px-4 py-1.5 bg-indigo-50 text-indigo-700 font-bold rounded-lg border border-indigo-100 text-sm">
                                        {tech.trim()}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="flex gap-4 sticky bottom-0 bg-white pt-4 border-t border-gray-100">
                            <button
                                onClick={() => setSelectedInternship(null)}
                                className="flex-1 py-4 border-2 border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-gray-50 transition-all uppercase tracking-wide"
                            >
                                Close
                            </button>
                            {getApplicationStatus(selectedInternship.id) ? (
                                <button
                                    disabled
                                    className="flex-[2] py-4 bg-gray-100 text-gray-400 rounded-xl font-bold uppercase tracking-wide cursor-not-allowed"
                                >
                                    Already Applied
                                </button>
                            ) : (
                                <button
                                    ...
                                    className="flex-[2] py-4 bg-primary text-white rounded-xl font-bold hover:bg-secondary transition-all shadow-xl hover:shadow-primary/40 uppercase tracking-wide"
                                >
                                    Apply for this Role
                                </button>
                            )}
                                ...
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => handleEditIdeaClick(item)}
                                            ...
                                            <button
                                                onClick={() => handleDeleteIdea(item.id || item.Id)}
            ...
            {activeTab === 'post-idea' && (
                <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-gray-100 animate-fade-in">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-dark">{isEditingIdea ? 'Edit Your Innovation' : 'Share Your Innovation'}</h3>
                        {isEditingIdea && (
                            <button
                                onClick={() => {
                                    setIsEditingIdea(false);
                    ...
                    <form onSubmit={handlePostIdea} className="space-y-4">
                        ...
                        <button type="submit" className="w-full py-2 bg-primary text-white rounded-lg hover:bg-secondary transition-colors font-semibold">
                            {isEditingIdea ? 'Update Idea' : 'Post Idea'}
                        </button>
                    </form>
                </div>
            )}
